...
export default function VoirPersonnel({retour,value,supprimerPer}) {
  ...
  return (
   ...
   <div className='absolute right-0 top-0 m-1 cursor-pointer  z-50 '>    
      ...
      <div className='flex flex-row '> 
        <button 
       type="button" onClick={()=>navigate(`/modifier/personnels/${value._id}`)} className=" text-green-700  border-r  font-medium  text-sm px-3 py-2 text-center inline-flex items-center">
        Modifier
        </button>
        <button 
       type="button" onClick={()=>supprimerPer(value._id)} className=" text-red-500  font-medium  text-sm px-3 py-2 text-center inline-flex items-center">
        Supprimer
        </button>
