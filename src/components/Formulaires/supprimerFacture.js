...
export default function SupprimerFacture({retour,value}) {
  ...
  return (
    <div className='w-[300px] space-y-4   items-center justify-center  border p-3 bg-white border-gray-100 shadow-md rounded-lg   z-10 absolute top-[200px] left-[50px]'>
     ...
     {chargement===false?<button type="button"  onClick={()=>supprimer()} className=" text-blue-400 hover:text-blue-700  font-medium  text-sm py-2.5 text-center inline-flex items-center  ">
        Supprimer
       
        </button> :<span className='flex flex-row items-center space-x-2 justify-center inline-flex items-center px-3 py-2 text-md font-medium text-center text-bleu-400'>
