...
.required()
  ...
  return (
   ...
   <p className='text-2xl font-semibold text-black tracking-wider mb-3'>Connectez-vous</p>             
    ...
    {error.nom&&<p className='text-sm font-semibold text-red-500 tracking-wider my-2'>{error.nom}</p> }
    <input {...register("password")} type='text' placeholder='Mot de passe' className='outline-none w-full border-b-2 py-1 text-lg'/>
      {error.password&&<p className='text-sm font-semibold text-red-500 tracking-wider my-2'>{error.password}</p> }
    </div> 
     </div>
